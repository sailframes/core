...
UD=$(sed -e "s|@@DATE@@|$DATE|g" -e "s|@@S3@@|$S3|g" -e "s|@@PREFIX@@|$PREFIX|g" -e "s|@@REGION@@|$REGION|g" \
        ...
        -e "s|@@HH0@@|$HH0|g" -e "s|@@HH1@@|$HH1|g" -e "s|@@SUFFIX@@|$SUFFIX|g" -e "s|@@BBOX@@|$BBOX|g" <<'USERDATA' | base64
#!/usr/bin/env bash
set -xeuo pipefail
...
/root/venv/bin/python "$SC" --prefix "$PREFIX" --domain "$DOMAIN" --cen-lat "$CEN_LAT" --cen-lon "$CEN_LON" \
  --hh0 "$HH0" --hh1 "$HH1" ${BBOX:+--bbox="$BBOX"} $COAST --scratch /mnt/g --outdir /root/gust > /root/gust_out.txt 2>&1
RC=$?
set -e
